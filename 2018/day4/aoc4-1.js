const SleepyGuard = data => {
	...
	let maxSlept = guardIds.map(id => guardMinuteMap.get(id).reduce(sum)).reduce(max);

	let sleepyGuardId = guardIds.find(id => guardMinuteMap.get(id).reduce(sum) === maxSlept);
